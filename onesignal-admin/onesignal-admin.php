...
{
  wp_enqueue_script('onesignal_admin_js', plugins_url('unosignal-admin.js', __FILE__));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST["submit"])) {
    if (isset($_POST['onesignal_app_id']) && !empty($_POST['onesignal_app_id'])) {
      update_option('onesignal_app_id', sanitize_text_field($_POST['onesignal_app_id']));
    }
    if (isset($_POST['onesignal_rest_api_key']) && !empty($_POST['onesignal_rest_api_key'])) {
      update_option('onesignal_rest_api_key', sanitize_text_field($_POST['onesignal_rest_api_key']));
    }
    $send_to_mobile = isset($_POST['onesignal_send_to_mobile']) ? 'on' : 'off';
    update_option('onesignal_send_to_mobile', $send_to_mobile);
  }
}
...
?>
  ...
  <div class="os-content">
    ...
    <form method="post">
      ...
      <p>
        <?php
        $appID = esc_attr(get_option('onesignal_app_id'));
        if (!empty($appID)) {
          echo '✅ ' . $appID;
        } else {
      ...
      <p>
        <?php
        $apiKey = esc_attr(get_option('onesignal_rest_api_key'));
        if (!empty($apiKey)) {
      ...
      <div class="checkbox-wrapper">
        <label for="send-to-mobile">
          <input id="send-to-mobile" type="checkbox" name="onesignal_send_to_mobile" <?php echo get_option('onesignal_send_to_mobile') === 'on' ? 'checked' : '' ?>>
